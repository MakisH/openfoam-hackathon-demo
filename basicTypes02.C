...
int main(int argc, char *argv[]) {
  ...
  Info << "Tutorial 02" << nl << tab << "Basic OpenFOAM data structures" << nl
       << tab << "Hash Tables-> key-value container and Hash set" << nl
       << endl; // Similar to unordered_map in stl of c++
  // É o dicionário do python
  // A HashTable is useful when you need to associate a key with a value and
  // retrieve it quickly, without caring about the order. This container can
  // also be Resized, shrinked, cleared and clearStorage
  ...
  // Be carefull of the operator(). It does find-or-create. So if it does not
  // find it will create
  ...
  // Remove an entry.
  const Switch erased = patchNameToIDMap.erase(
      "ffff"); // returns a bool with the success of the operation
  ...
  // insert() does not overwrite existing keys
  {
    ...
    Info << nl << "insert(\"b\", 99) succeeded? " << ok << nl
         << "Value for \"b\" is: " << patchNameToIDMap["b"] << nl;
  }
  ...
  // set() inserts or overwrites
  {
    ...
    Info << nl << "set(\"b\", 99) succeeded? " << ok << nl
         << "Value for \"b\" is: " << patchNameToIDMap["b"] << nl;
  }
  ...
  forAll(sortedKeys, i) {
    ...
    Info << "   " << key << " -> " << patchNameToIDMap[key] << nl;
  }
